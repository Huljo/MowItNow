...
function parseConfig(configStr) {
    ...
    var mowers_lines = lines.splice(1, lines.length-1); // Array of mowers
    console.log(lines, mowers_lines);
    config.topRightPos = {x:parseInt(lastCellLine[0]), y:parseInt(lastCellLine[1])}; // Define first line in config object
    config.mowers = [];
    for(var i=0; i<mowers_lines.length; i++) {
        if(i%2){
            var position = mowers_lines[i-1].split(' ');
            config.mowers.push({
                pos:{x:parseInt(position[0]), y:parseInt(position[1])},
                cardinal:position[2],
                instructions:mowers_lines[i].split('')
            });
        }
    }
    return config;
